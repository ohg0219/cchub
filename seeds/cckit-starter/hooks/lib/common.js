...
function buildFullContext(status) {
  if (!status) return '[PDCA] No status available';

  const lines = ['[PDCA Context] 현재 진행 중인 작업:'];
  const primary = status.primaryFeature;

  if (primary && status.features && status.features[primary]) {
    const d = status.features[primary];
    lines.push(`- Feature: ${primary}`);
    lines.push(`- Phase: ${d.phase} (${d.phaseNumber}/6)`);
    lines.push(`- Match Rate: ${(d.matchRate && d.matchRate > 0) ? d.matchRate + '%' : '-'}`);
    if (d.documents && Object.keys(d.documents).length > 0) {
      const docStr = Object.entries(d.documents).map(([k, v]) => `${k}(${v})`).join(', ');
      lines.push(`- Documents: ${docStr}`);
    }
  }

  if (status.history && status.history.length > 0) {
    const last = status.history[status.history.length - 1];
    const ts = last.timestamp ? last.timestamp.substring(0, 10) : '';
    lines.push(`- Last Action: ${last.action} (${ts})`);
  }

  if (status.activeFeatures && status.activeFeatures.length > 0) {
    const summary = status.activeFeatures.map(f => {
      const d = status.features && status.features[f];
      return d ? `${f}(${d.phase})` : f;
    }).join(', ');
    lines.push(`Active Features: ${summary}`);
  }
